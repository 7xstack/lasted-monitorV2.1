...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[Call Update] Database error:', error);
...
export async function GET() {
  return NextResponse.json(
    { error: 'GET method not supported. Use POST with vn parameter.' },
    { status: 405 }
  );
}
...
export async function PUT() {
  return NextResponse.json(
    { error: 'PUT method not supported. Use POST with vn parameter.' },
    { status: 405 }
  );
}
