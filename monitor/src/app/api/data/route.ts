...
export async function POST(request: NextRequest) {
  try {
    ...
    query += ') AND mvi.visit_date = @visit_date AND mvi.status = @status ORDER BY mvi.urgent_id DESC';
    ...
    // Add status parameter for filtering only 'รอ' status
    dbRequest.input('status', sql.NVarChar, 'รอ');
