...
export default function InterviewTable({ setting, visitData, sectionTitle }: InterviewTableProps) {
  console.log(visitData);
  const queueListContainerRef = useRef<HTMLDivElement>(null);
  const scrollIntervalRef = useRef<NodeJS.Timeout | null>(null);
  
  // Normal Mode: แสดงตารางแบบปกติ
  // เรียงลำดับ visitData ถ้าเปิด arr_r (table_arr2)
  ...
  // Auto scroll เมื่อคิวเกิน 24 คิว (เฉพาะ drug type)
  useEffect(() => {
    if (setting.type !== 'drug' || !queueListContainerRef.current) return;
  ...
  }, [sortedVisitData.length, setting.type]);
  ...
  // ถ้า type เป็น drug ให้แสดงเป็น div layout แทนตาราง (เหมือน queue type)
