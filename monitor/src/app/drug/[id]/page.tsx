...
export default function SinglePage({
  ...
  const [waitPrepareData, setWaitPrepareData] = useState<VisitInfo[]>([]); // สำหรับ "รอจัดยา"
  const [waitDispenseData, setWaitDispenseData] = useState<VisitInfo[]>([]); // สำหรับ "รอจ่ายยา"
    ...
    ws.onmessage = async (event) => {
      ...
      try {
        ...
        if (data.wait && Array.isArray(data.wait)) {
          ...
          // แยกข้อมูลตาม dept_name เฉพาะ drug type
          if (setting?.type === 'drug') {
            const prepareData = data.wait.filter((visit: VisitInfo) => 
              visit.dept_name === 'รอจัดยา'
            );
            const dispenseData = data.wait.filter((visit: VisitInfo) => 
              visit.dept_name === 'รอจ่ายยา'
            );
            setWaitPrepareData(prepareData);
            setWaitDispenseData(dispenseData);
          }
        }
  ...
  }, [currentDisplayId, audioUnlocked, setting]);
  ...
  return (
    <div className={styles.container} style={{ fontFamily }}>
      ...
      <main className={styles.mainContent}>
        ...
        {setting?.type === 'drug' ? (
          <>
            <InterviewTable setting={setting} visitData={waitPrepareData} sectionTitle="รอจัดยา" />
            <InterviewTable setting={setting} visitData={waitDispenseData} sectionTitle="รอจ่ายยา" />
          </>
        ) : (
          <>
            <InterviewTable setting={setting} visitData={visitData} sectionTitle="รอจัดยา" />
            <InterviewTable setting={setting} visitData={visitData} sectionTitle="รอจ่ายยา" />
          </>
        )}
