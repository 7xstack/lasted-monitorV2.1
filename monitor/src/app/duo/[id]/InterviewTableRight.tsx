...
export default function InterviewTable({ setting, visitData }: InterviewTableProps) {
  // Lock Position Mode: แสดงผลแบบ Card แนวตั้ง (ฝั่งขวา)
  if (setting.lock_position_right === "true" && setting.station_r) {
    // แยกสถานีจาก station_r
    const stationList = setting.station_r.split(",").map((s) => s.trim());
    
    return (
      <section className={styles.interviewSection}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}>
            {setting.n_room || 'จุดซักประวัติ'}
          </h2>
        </div>
  ...
  // เรียงลำดับ visitData ถ้าเปิด arr_r (table_arr2) - ฝั่งขวา
