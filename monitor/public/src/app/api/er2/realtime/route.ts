    ...
    async start(controller) {
      ...
      const fetchData = async () => {
        try {
          ...
          // Build query for visit data with JOIN to setting_urgent_level (ER2 logic)
          // - Select by dept_app IN (...)
          // - Exclude when code_dept_id is in the same set (or allow NULL)
          let visitQuery = `
            SELECT TOP 50 
              mvi.*,
          ...
          visitQuery += ') AND (mvi.code_dept_id IS NULL OR LTRIM(RTRIM(mvi.code_dept_id)) NOT IN (';
          departmentIds.forEach((id, index) => {
            if (index > 0) visitQuery += ',';
            visitQuery += `@dept${index}`;
          });
