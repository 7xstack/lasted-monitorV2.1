...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build the query for call data (status_call = 1) - multi department logic like patients API
    const placeholders = departmentIds.map((_, index) => `@dept${index}`).join(',');
    const query = `
      SELECT TOP 1 *
      FROM monitor_visit_info
      WHERE LTRIM(RTRIM(dept_app)) IN (${placeholders})
        AND (
          code_dept_id IS NULL
          OR LTRIM(RTRIM(code_dept_id)) NOT IN (${placeholders})
        )
        AND visit_date = @visit_date
        AND status_call = @status_call
