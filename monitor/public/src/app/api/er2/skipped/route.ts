...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build the query for skipped queue data (status = 'ข้าม') and JOIN with setting_urgent_level table
    // ER2 logic:
    // - Select by dept_app IN (...)
    // - Exclude rows where code_dept_id is in the same set (allow NULL)
    let query = `
      SELECT 
        mvi.*,
      ...
      WHERE LTRIM(RTRIM(mvi.dept_app)) IN (`;
    
    departmentIds.forEach((id, index) => {
      if (index > 0) query += ',';
      query += `@dept${index}`;
    });
    query += ') AND (mvi.code_dept_id IS NULL OR LTRIM(RTRIM(mvi.code_dept_id)) NOT IN (';
    departmentIds.forEach((id, index) => {
      if (index > 0) query += ',';
      query += `@dept${index}`;
    });
    query += ')) AND mvi.visit_date = @visit_date AND mvi.status = @status ORDER BY mvi.check_in DESC';
    ...
    dbRequest.input('status', sql.VarChar, 'ข้าม');
