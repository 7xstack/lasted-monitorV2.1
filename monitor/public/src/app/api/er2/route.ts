...
export async function POST(request: NextRequest) {
  try {
    ...
    const query = `
      ...
      WHERE mvi.dept_app = @dept
        AND mvi.code_dept_id <> @dept
        AND mvi.visit_date = @visit_date
      ORDER BY mvi.check_in`;

    // Execute query
    const dbRequest = connection.request();
    
    // Choose first department as target for equality/inequality condition
    const targetDept = departmentIds[0];
    dbRequest.input('dept', sql.VarChar, targetDept);
...
export async function GET() {
  return NextResponse.json(
    { error: 'GET method not supported. Use POST with department_load and visit_date parameters.' },
    { status: 405 }
  );
}
