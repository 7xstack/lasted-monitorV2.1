    ...
    async start(controller) {
      ...
      const fetchData = async () => {
        try {
          ...
          // Filter only status = 'รอ' for left side
          let visitQuery = `
            SELECT TOP 50 
              mvi.*,
          ...
          visitQuery += ') AND mvi.visit_date = @visit_date AND mvi.status = @status ORDER BY mvi.check_in';
          ...
          visitRequest.input('status', sql.NVarChar, 'รอ');
