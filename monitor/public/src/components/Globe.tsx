...
export default function Globe() {
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(new Color(0xe0f2ff), 0.85)
    renderer.domElement.style.background =
      "radial-gradient(circle at 25% 25%, rgba(59, 130, 246, 0.25), transparent 55%), radial-gradient(circle at 75% 75%, rgba(125, 211, 252, 0.3), transparent 60%), linear-gradient(135deg, rgba(248, 251, 255, 0.95), rgba(224, 242, 255, 0.9))"
   ...
   `
    ...
    const colors = [
      new Color(0x1d4ed8), // Royal Blue
      new Color(0x38bdf8), // Sky Blue
      new Color(0x60a5fa), // Light Indigo
      new Color(0x0ea5e9), // Cyan
      new Color(0x2563eb), // Cobalt
    ]
    let colorIndex = 0
  ...
  return (
    <div ref={mountRef} className="fixed top-0 left-0 w-full h-full z-0">
      {showHint && (
        <div className="absolute bottom-4 right-4 bg-white/80 text-sky-900 text-sm px-3 py-1 rounded-full shadow-lg shadow-sky-200 backdrop-blur-md transition-opacity duration-1000 opacity-80 hover:opacity-100">
          Drag to explore
        </div>
      )}
    </div>
