...
async function fetchDrugData(client, today) {
  try {
    info(`[DRUG] เริ่มดึงข้อมูลสำหรับ setting_id: ${client.setting_id}, date: ${today}`);
    ...
    info(`[DRUG] ดึง settings สำเร็จ - department_load: ${departmentLoadString}, style_voice: ${genderStyle}, voice: ${settings?.voice}`);

    // If no departments are assigned to this setting ID, stop here.
    if (!departmentLoadString) {
      warn(`[DRUG] ไม่มี departments สำหรับ setting_id ${client.setting_id}, ข้ามการ query`);
      client.send(
...
LEFT JOIN
    department AS T3 ON T1.code_dept_id = T3.code
WHERE 
...
ORDER BY
    ...
    info(`[DRUG] ผลลัพธ์จาก DB - wait: ${waitResult.recordset.length}, active: ${activeResult.recordset.length}, call: ${callData ? 'มี' : 'ไม่มี'}, skip: ${skipResult.recordset.length}`);
    
    // Debug: ตรวจสอบ dept_name ใน wait data
    if (waitResult.recordset.length > 0) {
      const sampleWait = waitResult.recordset[0];
      info(`[DRUG] Sample wait item - visit_q_no: ${sampleWait.visit_q_no}, dept_name: ${sampleWait.dept_name}, code_dept_id: ${sampleWait.code_dept_id}`);
      // ตรวจสอบ dept_name ทั้งหมด
      const deptNames = waitResult.recordset.map(r => r.dept_name).filter(Boolean);
      const uniqueDeptNames = [...new Set(deptNames)];
      info(`[DRUG] Unique dept_name values in wait data:`, uniqueDeptNames);
    }
    ...
    if (callData && settings) {
      ...
      if (!callData) {
        debug(`[DRUG] ไม่มี callData สำหรับส่งไป voice API`);
      }
      if (!settings) {
        warn(`[DRUG] ไม่มี settings สำหรับส่งไป voice API`);
      }
    }
    ...
    if (previousResponse === responseString) {
      ...
      debug(`[DRUG] ข้อมูลเหมือนเดิม ไม่ log`);
    } else {
      // ข้อมูลเปลี่ยน log และอัปเดต previous response
      info(`[DRUG] ✅ ส่งข้อมูลกลับไปยัง client สำเร็จ - wait: ${responseData.wait.length}, active: ${responseData.active.length}, call: ${responseData.call ? 'มี' : 'ไม่มี'}`);
      
      // แสดง response data จาก voice API ถ้ามี
      if (voiceApiResponseData) {
        info(`[DRUG] 📥 Response data จาก voice.aztecthstudio.com:`, voiceApiResponseData);
      }
    ...
    error(`[DRUG] ❌ Failed to fetch Drug data for setting_id ${client.setting_id}:`, err);
    // Optionally send an error message to the client
    client.send(JSON.stringify({ error: "Failed to fetch Drug data" }));
  }
}

export default fetchDrugData;
