...
async function fetchErData(client, today) {
  try {
    ...
    const waitQuery = departments.length > 0
      ? `${baseQuery} WHERE T1.code_dept_id IN (${deptPlaceholders}) AND T1.visit_date = @visit_date AND T1.status = N'รอ' ORDER BY T2.priority_rate DESC`
      : null;
    const activeQuery = departments.length > 0
      ? `${baseQuery} WHERE T1.code_dept_id IN (${deptPlaceholders}) AND T1.visit_date = @visit_date AND T1.status = N'กำลัง' ORDER BY T2.priority_rate DESC, T1.time_call ASC`
      : null;

    const callQuery =
      departments.length > 0
        ? `SELECT TOP(1) T1.*, T2.Urgent_Level as urgent_level, T2.Description as urgent_description, T2.notice_text as urgent_notice_text, T3.pname as pname FROM monitor_visit_info AS T1 INNER JOIN setting_urgent_level AS T2 ON T1.urgent_id = T2.ID LEFT JOIN visit_info AS T3 ON T1.vn = T3.vn WHERE T1.visit_date = @visit_date AND T1.status_call = '1' AND T1.code_dept_id IN (${deptPlaceholders}) ORDER BY T2.priority_rate DESC, T1.time_call ASC`
        : null;
    const skipQuery =
      departments.length > 0
        ? `SELECT * FROM monitor_visit_info WHERE visit_date = @visit_date AND status = N'ข้าม' AND code_dept_id IN (${deptPlaceholders})`
        : null;
