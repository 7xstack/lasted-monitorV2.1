...
async function fetchTripleData(client, today) {
  try {
    info(`[TRIPLE] เริ่มดึงข้อมูลสำหรับ setting_id: ${client.setting_id}, date: ${today}`);
    ...
    info(`[TRIPLE] ดึง settings สำเร็จ - department_load: ${departmentLoadString}, style_voice: ${genderStyle}, voice: ${settings?.voice}`);

    // If no departments are assigned to this setting ID, stop here.
    if (!departmentLoadString) {
      warn(`[TRIPLE] ไม่มี departments สำหรับ setting_id ${client.setting_id}, ข้ามการ query`);
      client.send(
...
ORDER BY
    ...
    info(`[TRIPLE] ผลลัพธ์จาก DB - wait: ${waitResult.recordset.length}, active: ${activeResult.recordset.length}, call: ${callData ? 'มี' : 'ไม่มี'}, skip: ${skipResult.recordset.length}`);
    ...
    if (callData && settings) {
      ...
      if (!callData) {
        debug(`[TRIPLE] ไม่มี callData สำหรับส่งไป voice API`);
      }
      if (!settings) {
        warn(`[TRIPLE] ไม่มี settings สำหรับส่งไป voice API`);
      }
    }
    ...
    if (previousResponse === responseString) {
      ...
      debug(`[TRIPLE] ข้อมูลเหมือนเดิม ไม่ log`);
    } else {
      // ข้อมูลเปลี่ยน log และอัปเดต previous response
      info(`[TRIPLE] ✅ ส่งข้อมูลกลับไปยัง client สำเร็จ - wait: ${responseData.wait.length}, active: ${responseData.active.length}, call: ${responseData.call ? 'มี' : 'ไม่มี'}`);
      
      // แสดง response data จาก voice API ถ้ามี
      if (voiceApiResponseData) {
        info(`[TRIPLE] 📥 Response data จาก voice.aztecthstudio.com:`, voiceApiResponseData);
      }
    ...
    error(`[TRIPLE] ❌ Failed to fetch Triple data for setting_id ${client.setting_id}:`, err);
    // Optionally send an error message to the client
    client.send(JSON.stringify({ error: "Failed to fetch Triple data" }));
  }
}

export default fetchTripleData;
